...
const GoldTicker = () => {
  ...
  useEffect(() => {
    if (current) {
      const price24K = parseFloat(current.gold24KRate);
      const price22K = parseFloat(current.gold22KRate);
      
      const prevPrice = previous ? parseFloat(previous.gold24KRate) : price24K;
      const change = price24K - prevPrice;
      const percentage = (change / prevPrice) * 100;

      setTickerItems([
        { label: 'GOLD 24K', value: `₹${price24K.toLocaleString()}`, change, percentage, icon: '🟡' },
        { label: 'GOLD 22K', value: `₹${price22K.toLocaleString(undefined, { maximumFractionDigits: 0 })}`, change: change * 0.916, percentage, icon: '🟠' },
        { label: 'SILVER', value: `₹88,450/kg`, change: 120, percentage: 0.14, icon: '⚪' },
