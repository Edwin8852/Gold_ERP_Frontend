...
const LoanPaymentsCollection = () => {
  ...
  useEffect(() => {
    if (searchQuery.trim() === '') {
      ...
      const filtered = loans.filter(l => 
        ...
        (l.customer && l.customer.customerCode?.toLowerCase().includes(q))
      );
  ...
  const handleDownloadReceipt = async (paymentId) => {
    try {
      ...
      link.download = `Receipt-${paymentId.split('-')[0]}.pdf`;
      link.click();
  ...
  return (
    <div className="container mx-auto p-4 lg:p-6 space-y-8 bg-gray-50/50 min-h-screen">
            ...
            {selectedLoan && (
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
                  ...
                  <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
                    <span className="text-[10px] font-bold text-gray-400 block uppercase">{t('Interest Due')}</span>
                    <span className="text-base font-bold text-amber-500 font-mono">₹{parseFloat(selectedLoan.interestAmount).toLocaleString('en-IN')}</span>
                  </div>
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center p-3 bg-white/5 rounded-xl border border-white/5">
                    <span className="text-xs text-gray-300">{t('Penalty Component Covered')}</span>
                    <span className="font-bold text-red-300 font-mono">+ ₹{simulation.penaltyPaid.toFixed(2)}</span>
                  </div>

                  <div className="flex justify-between items-center p-3 bg-white/5 rounded-xl border border-white/5">
                    <span className="text-xs text-gray-300">{t('Interest Component Covered')}</span>
                    <span className="font-bold text-amber-300 font-mono">+ ₹{simulation.interestPaid.toFixed(2)}</span>
                  </div>

                  <div className="flex justify-between items-center p-3 bg-white/5 rounded-xl border border-white/5">
                    <span className="text-xs text-gray-300">{t('Principal Component Covered')}</span>
                    <span className="font-bold text-emerald-300 font-mono">+ ₹{simulation.principalPaid.toFixed(2)}</span>
                  </div>
                </div>

                <div className="pt-3 border-t border-white/10 flex justify-between items-center">
                  <span className="text-xs font-bold text-gold">{t('NEW Outstanding Total')}</span>
