...
const MyLoans = () => {
  ...
  const handleDownloadInvoice = async (invoiceNumber) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/api/invoices/download-by-number/${invoiceNumber}`, {
        headers: {
      ...
      <div className="grid grid-cols-1 gap-6">
        {loans.length > 0 ? loans.map((loan, index) => (
            ...
            <div className="flex flex-col md:flex-row justify-between gap-6 md:gap-8 relative z-10">
              ...
              <div className="flex flex-col justify-between items-start md:items-end">
                <div className="text-left md:text-right">
                  ...
                  <h4 className="text-2xl md:text-3xl font-black text-gold">₹ {loan.remainingPrincipal?.toLocaleString() || loan.loanAmount?.toLocaleString()}</h4>
                </div>
                <div className="flex flex-wrap gap-3 mt-4">
                    ...
                    >
                      {expandedLoanId === loan.id ? t('Hide') : t('Details')} <ChevronRight size={18} className={`transform transition-transform ${expandedLoanId === loan.id ? 'rotate-90' : ''}`} />
                    </button>
                </div>
              </div>
            </div>

            {/* Expandable Loan Details & Payment Timeline */}
            <AnimatePresence>
              {expandedLoanId === loan.id && (
                <motion.div
                  ...
                  className="overflow-hidden border-t border-gray-100 mt-8 pt-8 relative z-10"
                >
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 bg-gray-50 p-6 rounded-3xl">
                    <div>
                      <p className="text-xs text-gray-400 font-bold uppercase">{t('Total Principal Paid')}</p>
                      <p className="text-lg font-extrabold text-gray-800">₹ {Number(loan.totalPrincipalPaid || 0).toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-400 font-bold uppercase">{t('Total Interest Paid')}</p>
                      <p className="text-lg font-extrabold text-purple-700">₹ {Number(loan.totalInterestPaid || 0).toLocaleString()}</p>
                    </div>
                    <div>
                      <p className="text-xs text-gray-400 font-bold uppercase">{t('Total Penalty Paid')}</p>
                      <p className="text-lg font-extrabold text-rose-600">₹ {Number(loan.totalPenalty || 0).toLocaleString()}</p>
                    </div>
                    <div className="md:col-span-3 grid grid-cols-2 gap-4 border-t border-gray-200/50 pt-4 text-xs text-gray-500">
                      <div>
                        <span className="font-bold">{t('Last Payment:')}</span> {loan.lastPaymentDate ? new Date(loan.lastPaymentDate).toLocaleString() : t('None')}
                      </div>
                      <div>
                        <span className="font-bold">{t('Next Due Date:')}</span> {loan.nextDueDate ? new Date(loan.nextDueDate).toLocaleDateString() : new Date(loan.dueDate).toLocaleDateString()}
                      </div>
                    </div>
                  </div>
                  ...
                  <h4 className="text-lg font-bold mb-4 flex items-center gap-2">
                    ...
                    {t('Payment History & Invoice Timeline')}
                  </h4>
                  ...
                  {loan.payments && loan.payments.length > 0 ? (
                      ...
                      <table className="w-full text-left text-sm text-gray-500">
                        <thead className="text-xs text-gray-700 uppercase bg-gray-100 rounded-xl">
                          <tr>
                            ...
                            <th className="px-4 py-3">{t('Breakdown')}</th>
                        ...
                        <tbody>
                          {loan.payments.map((pmt) => (
                            <tr key={pmt.id} className="border-b border-gray-150 hover:bg-gray-50/50">
                              <td className="px-4 py-4 font-medium text-gray-900">
                                {new Date(pmt.paymentDate || pmt.createdAt).toLocaleDateString()}
                              </td>
                              <td className="px-4 py-4 font-bold text-gray-900">
                                ₹ {Number(pmt.paymentAmount || 0).toLocaleString()}
                              </td>
                              <td className="px-4 py-4 text-xs">
                                <div className="space-y-1">
                                  <div><span className="text-gray-400 font-bold">{t('Principal:')}</span> ₹ {Number(pmt.principalPaid || pmt.paymentAmount || 0).toLocaleString()}</div>
                                  <div><span className="text-purple-400 font-bold">{t('Interest:')}</span> ₹ {Number(pmt.interestPaid || 0).toLocaleString()}</div>
                                  <div><span className="text-rose-400 font-bold">{t('Penalty:')}</span> ₹ {Number(pmt.penaltyPaid || pmt.penaltyAmount || 0).toLocaleString()}</div>
                                </div>
                              </td>
                              <td className="px-4 py-4">
                                <span className={`px-2 py-1 rounded-full text-[10px] font-bold ${getStatusBadgeClass(pmt.paymentStatus || 'ACTIVE')}`}>
                              ...
                              <td className="px-4 py-4">
                                {pmt.invoiceNumber && (
                                  <button
                                    onClick={() => handleDownloadInvoice(pmt.invoiceNumber)}
                                    className="flex items-center gap-1.5 bg-gold/10 text-gold hover:bg-gold/20 px-3 py-1.5 rounded-xl font-bold text-xs transition-all"
                                  >
                                    {t('Invoice PDF')}
                                  </button>
                                )}
                    ...
                    <p className="text-sm text-gray-400 bg-gray-50 p-6 rounded-2xl text-center font-medium">
                      {t('No payments processed yet for this loan.')}
                    </p>
                  )}
                </motion.div>
              )}
          ...
          <div className="glass-card p-20 rounded-[2.5rem] text-center">
            ...
            <h3 className="text-xl font-bold text-gray-900">{t('No active loans')}</h3>
            <p className="text-gray-500 mt-2">{t("You don't have any gold loans at the moment.")}</p>
