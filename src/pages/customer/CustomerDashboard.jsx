...
const CustomerDashboard = () => {
  ...
  const stats = myDashboard?.overview || {
    totalLoans: 0,
    activeChits: 0,
    riskScore: 'A+'
  };
  ...
  const chits = myDashboard?.chits || [];
  const recentPayments = myDashboard?.recentPayments || [];

  return (
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-display font-bold">{t('Welcome back')}, {user?.firstName || 'Customer'}! 👋</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
        <div className="bg-gradient-to-br from-gold to-amber-600 p-8 rounded-[2.5rem] text-black shadow-gold/20 shadow-2xl relative overflow-hidden group">
          ...
          <p className="text-sm font-bold uppercase tracking-wider opacity-80">{t('Active Loans')}</p>
          <h3 className="text-4xl font-black mt-2">{stats.totalLoans}</h3>
          <div className="mt-8 flex items-center justify-between">
            <span className="text-xs font-bold bg-black/10 px-3 py-1.5 rounded-full">{t('Secure & Insured')}</span>
            <Link to="/customer/loans" className="flex items-center gap-1 text-sm font-bold group">
              {t('View All')} <ChevronRight size={16} className="group-hover:translate-x-1 transition-transform" />
            </Link>
          </div>
        </div>

        <div className="glass-card p-8 rounded-[2.5rem] flex flex-col justify-between relative overflow-hidden">
          <div className="absolute -right-4 -top-4 w-24 h-24 bg-blue-500/5 rounded-full blur-2xl" />
          <div>
            <div className="w-12 h-12 bg-blue-500/10 rounded-2xl flex items-center justify-center text-blue-500 mb-6 shadow-inner">
              <Wallet size={24} />
            </div>
            <p className="text-sm text-gray-500 font-medium">{t('Active Chits')}</p>
            <h3 className="text-3xl font-bold mt-1 tracking-tight">{stats.activeChits}</h3>
          </div>
          <div className="flex items-center justify-between mt-4">
             <p className="text-xs text-blue-600 font-bold uppercase tracking-widest">{t('Contributing Regularly')}</p>
        ...
        <div className="glass-card p-8 rounded-[2.5rem] flex flex-col justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 font-medium">{t('Credit Score')}</p>
            <h3 className="text-3xl font-bold mt-1">{t('Excellent')}</h3>
          </div>
          <div className="w-full bg-gray-100 dark:bg-dark-card h-2 rounded-full mt-4 overflow-hidden">
            ...
            {chits.length > 0 && chits.map((chit) => (
               <div key={chit.id} className="flex items-center justify-between p-4 md:p-6 bg-blue-50/30 dark:bg-blue-500/5 rounded-3xl border border-blue-100/50 dark:border-blue-500/10 hover:border-blue-500/30 transition-all cursor-pointer group">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-black text-gray-900">{chit.scheme?.schemeName}</p>
                      <p className="text-xs text-gray-500 font-medium">Ticket #{chit.ticketNumber} • {chit.scheme?.durationMonths} {t('Months Plan')}</p>
                    </div>
                  </div>
            ...
            {loans.length > 0 ? loans.map((loan) => (
              <div key={loan.id} className="flex items-center justify-between p-6 bg-gray-50 dark:bg-dark-card rounded-3xl border border-transparent hover:border-gold/30 transition-all cursor-pointer">
                ...
                <div className="text-right">
                  ...
                  <p className={`text-xs font-bold ${loan.status === 'ACTIVE' ? 'text-green-500' : 'text-gray-500'}`}>
                    {loan.status}
                  </p>
                </div>
              </div>
