  ...
  return (
    <div className="space-y-10 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-display font-bold tracking-tight">Gold Finance Dashboard</h1>
          <div className="flex items-center gap-3 mt-1">
            <p className="text-gray-500">Manage your active loans and tracking pending applications.</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: 'Total Borrowed', value: `₹${stats.totalBorrowed.toLocaleString()}`, icon: Coins, color: 'bg-gold' },
          { label: 'Active Loans', value: stats.totalActive, icon: ShieldCheck, color: 'bg-green-500' },
          { label: 'Pending Requests', value: stats.pending, icon: Clock, color: 'bg-blue-500' },
        ].map((stat, i) => (
          ...
          {loading && loans.length === 0 ? (
              ...
              <div className="flex items-center gap-6 w-full md:w-auto">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    {loan.goldWeight}g • {loan.goldPurity} Gold • {new Date(loan.loanDate).toLocaleDateString()}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between w-full md:w-auto md:gap-12">
                <div className="text-right">
                  <p className="text-xs text-gray-400 font-bold uppercase tracking-widest">
                    {loan.status === 'PENDING_APPROVAL' ? 'Requested Amount' : 'Approved Amount'}
                  </p>
            ...
            <div className="glass-card p-20 rounded-[3rem] text-center space-y-6">
              ...
              <div className="space-y-2">
                <h4 className="text-xl font-bold">No loans found</h4>
                <p className="text-gray-500">You haven't applied for any gold loans yet.</p>
              </div>
              <button 
                onClick={() => navigate('/customer/services/loan')}
