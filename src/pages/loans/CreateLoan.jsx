...
const loanSchema = z.object({
  ...
  marketRate: z.string().min(1, "Market rate is required"),
...
const CreateLoan = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-12">
      ...
      {/* Modern Tabs */}
      <div className="flex border-b border-gray-100 dark:border-dark-border mb-8">
        <button 
          onClick={() => setActiveTab('disburse')}
          className={`flex-1 py-4 text-center font-black uppercase tracking-wider text-xs border-b-2 transition-all ${
        ...
        <button 
          ...
          className={`flex-1 py-4 text-center font-black uppercase tracking-wider text-xs border-b-2 transition-all ${
            activeTab === 'ledger' ? 'border-gold text-gold border-b-gold font-extrabold' : 'border-transparent text-gray-400 hover:text-gray-600'
          }`}
        >
          {t('Loan History & Ledger')}
        </button>
      </div>
                ...
                <AnimatePresence mode="wait">
                  ...
                  {step === 2 && (
                    <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <Input label={t('Current Market Rate (per gram)')} type="number" {...register('marketRate')} error={errors.marketRate?.message ? t(errors.marketRate.message) : undefined} />
                      </div>
                  ...
                  {step === 3 && (
                    <motion.div key="step3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <Input label={t('Final Loan Amount (₹)')} type="number" {...register('loanAmount')} error={errors.loanAmount?.message ? t(errors.loanAmount.message) : undefined} />
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white dark:bg-dark-card p-6 rounded-[2rem] border border-gray-100 dark:border-dark-border shadow-sm">
              ...
              <div className="text-xs text-gray-500 font-bold uppercase tracking-wider">
                {t('Total Records:')} {filteredLoans.length}
              </div>
            </div>
                    ...
                    {filteredLoans.length === 0 ? (
                      <tr>
                        <td colSpan="7" className="px-8 py-20 text-center text-gray-500 font-medium">
                          {t('No gold loans found in the ledger history.')}
                        </td>
                      </tr>
                    ) : (
